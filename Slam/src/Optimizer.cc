...
#define ObjectFixScale // use when scale is provided and fixed. such as KITTI
    ...
    typedef g2o::EdgeSE3CuboidProj g2o_camera_obj_2d_edge;
        ...
        for (vector<MapObject *>::iterator lit = lLocalMapObjects.begin(),
            ...
            if (scene_unique_id == kitti) {
                if (!build_worldframe_on_ground) {
                    float cam_height = Converter::toCvMat(pKF->GetCameraCenter()).at<float>(
                    ...
                    cube_pose.setTranslation(Eigen::Vector3d(cube_pose.translation()(0), cam_height + 1.0,
                                                      cube_pose.translation()(2)));
                } else {
                    float cam_height = Converter::toCvMat(pKF->GetCameraCenter()).at<float>(2);
                    cube_pose.setTranslation(Eigen::Vector3d(cube_pose.translation()(0),
        ...
        if (1) {
            ...
            vector<vector<Eigen::Vector3d>> all_object_ba_points(lLocalMapObjects.size());
             ...
             lit != lend; lit++)
        {
                 ...
                 mit != mend; mit++)
            {
                ...
                if (scene_unique_id == kitti)
                {
                    if (!build_worldframe_on_ground)
                    {
                        float cam_height = Converter::toCvMat(pKFi->GetCameraCenter()).at<float>(
                                1); // in init frame, y downward for KITTI
                        // 		  cube_pose.setTranslation(Vector3d(cube_pose.translation()(0),1.7-0.7631,cube_pose.translation()(2)));
                        cube_pose.setTranslation(Eigen::Vector3d(cube_pose.translation()(0), cam_height + 1.0,
                                                          cube_pose.translation()(2)));
                    ...
                    {
                        float cam_height =
                                Converter::toCvMat(pKFi->GetCameraCenter()).at<float>(2); // in init frame, y downward for KITTI
                    }
                 ...
                 lit != lend; lit++)
            {
                    ...
                    firstgoodobsframe && lastgoodobsframe)
                {
                    ...
                    double linear_esti =
                            ...
                            time_range;
                    double yaw_from = init_pose.pose.toMinimalVector()(5);
                    double yaw_to = last_pose.pose.toMinimalVector()(5);
                    double angle_error = g2o::normalize_theta(yaw_to - yaw_from);
                    pMObject->velocityPlanar = Eigen::Vector2d(linear_esti, 0);
