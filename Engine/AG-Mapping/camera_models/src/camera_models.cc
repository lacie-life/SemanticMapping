                                          ...
                                          int flags)
{
    ...
    for (int k = 0; k < 6; ++k)
    {
        for (int dilations = minDilations; dilations <= maxDilations; ++dilations)
        {
            ...
            cv::Mat kernel2 = cv::getStructuringElement(cv::MORPH_CROSS, cv::Size(3,3), cv::Point(1,1));
                                     ...
                                     cv::Size patternSize)
{
    ...
    while ((int)quadGroup.size() > count)
    {
        ...
        for (size_t skip = 0; skip < quadGroup.size(); ++skip)
        {
            ...
            // remember the smallest box area
                           ...
                           cv::Size patternSize, bool firstRun)
{
    ...
    //        size was reached in one direction -> We can check, whether the target
