...
void MainWindow::connection() {
    ...
    connect(ui->btn_continue, &QPushButton::clicked,
            this, &MainWindow::continueClicked);

    // force quit
    connect(ui->btn_forcequit, &QPushButton::clicked,
            this, &MainWindow::forcequitClicked);
...
void MainWindow::loadImages(const std::string &strAssociationDFilename, const std::string &strAssociationRGBFilename) {
    ...
    while (!fAssociationD.eof()) {
        ...
        if (!s_d.empty()) {
            ...
            ss >> t;
            ss >> sD;
            this->m_vstrImageFilenamesD.push_back(sD);
            CONSOLE << "Depth: " << QString::fromStdString(sD);

            std::string s;
            getline(fAssociationRGB, s);
            CONSOLE << QString::fromStdString(s);
            if (!s.empty()) {
                std::stringstream ss;
                ss << s;
                double t;
                ...
                ss >> sRGB;
                this->m_vstrImageFilenamesRGB.push_back(sRGB);
                CONSOLE << "RGB: " << QString::fromStdString(sRGB);
            }
        }
    }

    CONSOLE << "Depth: " << m_vstrImageFilenamesD.size();
    CONSOLE << "RGB: " << m_vstrImageFilenamesRGB.size();
}
...
void MainWindow::createCVWins() {
    ...
    auto img = cv::imread("../Engine/imgs/depth.png", cv::IMREAD_UNCHANGED);
    cv::imshow(this->m_rebuilder->m_cvWinName.c_str(), img);
    ...
    QTimer::singleShot(0, this, [=]() {
        ...
        auto img = cv::imread("../Engine/imgs/recognizer.png", cv::IMREAD_UNCHANGED);
        cv::imshow(this->m_recognizer->m_cvWinName.c_str(), img);
    });
}

void MainWindow::saveCliked()
{
...
void MainWindow::createSLAMFinished(float scale)
{
    // show message
...
void MainWindow::runSLAMClicked()
{
...
void MainWindow::processNewFrameFinishedDisplay(Sophus::SE3f pose)
{
...
void MainWindow::forcequitClicked()
{
    if (this->m_curFrameIdx < 0) {
        return;
    }
