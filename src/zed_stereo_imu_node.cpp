//
// Created by lacie on 22/04/2023.
//
...
class ImageGrabber {
public:
    ImageGrabber(semantic_slam::System *pSLAM, ImuGrabber *pImuGb, const bool bRect, const bool bClahe) : mpSLAM(pSLAM),
                                                                                                          mpImuGb(pImuGb),
                                                                                                          do_rectify(
                                                                                                                  bRect),
                                                                                                          mbClahe(bClahe) {}
...
int main(int argc, char **argv) {
    ...
    if (argc < 4 || argc > 5) {
        cerr << endl
             << "Usage: rosrun object_slam zed_stereo_node path_to_vocabulary path_to_settings do_rectify [do_equalize]"
             << endl;
        ros::shutdown();
        return 1;
    }
    ...
    semantic_slam::System SLAM(argv[1], argv[2], semantic_slam::System::IMU_STEREO, true);
    ...
    ros::Subscriber sub_img_left = n.subscribe("/zed2/zed_node/left/image_rect_color", 100,
                                               &ImageGrabber::GrabImageLeft, &igb);
    ros::Subscriber sub_img_right = n.subscribe("/zed2/zed_node/right/image_rect_color", 100,
...
void ImageGrabber::SyncWithImu() {
    ...
    while (1) {
        ...
        if (!imgLeftBuf.empty() && !imgRightBuf.empty() && !mpImuGb->imuBuf.empty()) {
            ...
            vector<semantic_slam::IMU::Point> vImuMeas;
            ...
            if (!mpImuGb->imuBuf.empty()) {
                ...
                while (!mpImuGb->imuBuf.empty() && mpImuGb->imuBuf.front()->header.stamp.toSec() <= tImLeft) {
                    ...
                    vImuMeas.push_back(semantic_slam::IMU::Point(acc, gyr, t));
                    mpImuGb->imuBuf.pop();
                }
            }
