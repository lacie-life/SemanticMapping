...
void ObjectDatabase::addObject(AG_MAPPING::Object_Map* object)
{
    // 1. Check the total quantity, if the database is empty, join directly
    if(!mObjects.size())
    {
    ...
    else
    {
        ...
        std::vector<AG_MAPPING::Object_Map*> likely_obj;
        ...
        // 3. If not found, add it directly to the database
        AG_MAPPING::Object_Map* best_close;// most recent index
        ...
        else // Find multiple objects with the same name in the database
        {
            ...
            for(unsigned int j = 0; j < likely_obj.size(); j++)
            {
                AG_MAPPING::Object_Map* temp_obj = likely_obj[j];
...
std::vector<AG_MAPPING::Object_Map*> ObjectDatabase::getObjectMapByName(int objectName)
{
    std::vector<AG_MAPPING::Object_Map*> likely_obj;
    for (int i = 0; i < mObjects.size(); i++)
    {
...
std::vector<AG_MAPPING::Object_Map*> ObjectDatabase::getAllObject()
{
    return std::vector<AG_MAPPING::Object_Map*>(mObjects.begin(), mObjects.end());
}
